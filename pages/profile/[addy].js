...
export default function V3Phunks() {
  ...
  //get pending withdrawals amount for connected address
  (async () => {
    const amt = await contract.pendingWithdrawals(connectedAddress);
    setPendingWithdrawAmt(amt);
  })();

  //withdraw
  async function withdraw() {
    const withdrawPromise = contract.withdraw();
        ...
        <div className="content mx-4">
          ...
          {connectedAddress === walletAddy && pendingWithdrawAmt > 0 ? 
            <div
              ...
              onClick={withdraw}
            >
              <button className="cta v3-b black-bg v3-txt">WITHDRAW {pendingWithdrawAmt}Ξ</button>
            </div>
            : null
          }
