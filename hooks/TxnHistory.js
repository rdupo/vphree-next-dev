...
const getTxnHistory = (contractAddress, id) => {
  ...
  useEffect(() => {
    const fetchTransactionHistory = async () => {
      ...
      // Fetch events for Mint, Sales, and Transfer
      const mintEvents = await nftContract.queryFilter(nftContract.filters.Transfer(null, null, null), 0, 'latest');
      //const transferEvents = await nftContract.queryFilter(nftContract.filters.Transfer(null, null, null), 0, 'latest');
      // Add other event types as needed

      const allEvents = [...mintEvents]; // Combine events as needed
      const filteredEvents = allEvents.filter(event => event.args.tokenId.toString() === id);
      console.log(filteredEvents);
      const formattedEvents = filteredEvents.map(event => ({
        from: event.args.from,
        to: event.args.to,
        tokenId: event.args.tokenId.toString(),
        eventType: event.args.from == '0x0000000000000000000000000000000000000000' ? 'Mint' : event.event,
      }));
